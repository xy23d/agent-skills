...
集計はスクリプトに委譲する。`target`（複数可）配下の日付別ログ（`<date>.log`）を `lookback_days` 範囲で読み、全 target をまたいで1つのコマンド頻度ランキングを出す。スキル起動時に示されるベースディレクトリを使って実行する。

```bash
bash {BASE_DIR}/scripts/scan-commands.sh
```

出力は `回数<TAB>コマンド` の一覧（頻度の降順）。生の頻度であり、noise 除去はしていない。

注意: これは**完全一致**の頻度集計のため、引数・パス違いで散らばる類似作業（例: `git -C /a status` と `git -C /b status`）は別カウントになり集約されない。頻度ランキングは補助シグナルとして使い、意味的な集約は Step3-4 で行う。

---

### Step 3 — ログから関連パターンを抽出する

`target` 配下の日付別ログ（`<date>.log`）を `lookback_days` 範囲で読み、反復している作業を意味的に抽出する。Step 2 の頻度ランキングは、完全一致で繰り返されている作業を素早く見つける**補助シグナル**として併用する（起点にはしない）。引数・パス違いで散らばる類似作業は頻度ランキングに浮上しないため、ログ本文から意味的に拾う。
...
`scan-commands.sh` の出力は生の頻度であり、`ls` / `cd` のような定型コマンドが上位に来やすい。
