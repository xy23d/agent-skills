...
### Step 2 — Markdownファイルを収集する（機械処理）

収集はスクリプトに委譲する。include / exclude / lookback_days / max_file_size_kb / max_files の適用はすべてスクリプト側で行う。スキル起動時に示されるベースディレクトリを使って実行する。

```bash
bash {BASE_DIR}/scripts/scan.sh
```

出力は `path<TAB>更新日<TAB>サイズKB` の一覧（更新日の新しい順）。

README・CHANGELOG等の静的ドキュメントは targets.md の exclude で管理する。それでも混ざった場合は低優先度として扱う。

---

### Step 3 — 過去N日分の関連エントリを抽出する

Step 2 の一覧にあるファイルを読み、関連する記述を抽出する。

エントリの日付は、scan.sh が出力するファイル更新日を基本とし、本文中に明示された日付（見出し・frontmatter・箇条書き・ファイル名）があればそちらを優先する。日付が明示されていないエントリは確度を低めに評価する。
