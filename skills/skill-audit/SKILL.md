...
`targets.md` の `usage_log` にスキル使用ログ（フックが `日付<TAB>スキル名` 形式で収集）のパスが
指定されている場合、existing_assets のスキル一覧と突き合わせる。

- ログに一度も現れない、または最終使用日が古い（目安: 30日超）スキル → prune・統合候補として通知に含める
- ログ収集スクリプトは `scripts/log-skill-usage.sh`（PostToolUse(Skill) フックとして登録して使う。
  登録パスは環境側の settings.json に書く）
- `usage_log` が未指定またはファイルが存在しない場合はこの照合をスキップする
