...
各バックエンドの利用可能モデル一覧は `.model-cache/<backend>-models.json` に週次キャッシュする。このキャッシュは生成物なので git 追跡しない。実行スクリプトの冒頭で、キャッシュの mtime の ISO 週が今週ならそのまま委譲し、キャッシュ不在または週が変わっている場合だけ CLI の正規手段で再取得する。

再取得後、利用可能モデル一覧と `model-tiers.tsv` に差分があれば stderr に警告する。階層表にあるモデルが一覧から消えている場合は retirement 候補、一覧にあるモデルが階層表に無い場合は未分類として扱う。警告だけなら委譲は続行する。

モデル一覧の取得に失敗した場合は fail-closed とし、古いキャッシュで続行せず委譲を実行しない。CLI にモデル一覧取得の正規手段が無いバックエンドも同じく、再取得が必要な時点で非0終了する。
